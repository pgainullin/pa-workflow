...
self.llama_parser = LlamaParse(
    ...
    model="openai-gpt-4-1-mini",  # ← Added this line
...
The `model` parameter specifies **which LLM** powers this agent. The `openai-gpt-4-1-mini` model is:
...
The agent now:
- Uses a proven, optimized LLM (GPT-4.1 Mini)
...
llama_parser = LlamaParse(
    parse_mode="parse_page_with_agent",
    model="openai-gpt-4-1-mini",  # ← Now included!
    high_res_ocr=True,
    adaptive_long_table=True,
    outlined_table_extraction=True,
    output_tables_as_HTML=True,
